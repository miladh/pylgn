...
response = np.zeros([k_max_id, len(mask_size)]) / pq.s
...
for j, d in enumerate(mask_size):
    for i, k_d in enumerate(spatial_freqs):
        # create stimulus
        stimulus = pylgn.stimulus.create_patch_grating_ft(wavenumber=k_d, mask_size=d)
...
# visualize
for d, R in zip(mask_size, response.T):
